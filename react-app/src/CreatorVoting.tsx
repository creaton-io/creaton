...
export const CreatorVoting: FC = () => {
  ...
  async function handleSubmit(e) {
    ...
    const creatorVotingFactoryContract: Contract = new ethers.Contract(
      CREATOR_VOTING_ADDRESS,
      creaton_contracts.creator_voting_factory.abi,
      signer
    );
    try {
      await creatorVotingFactoryContract.createVotingProcess(
      ...
      creatorVotingFactoryContract.once(
        'VotingProcessDeployed',
        async (creator, votingProcessAddress, question, description, uri, answers, acceptedTokens) => {
          setCreateVotingProcessesVisible(false);
          web3utils.setIsWaiting(false);
          notificationHandler.setNotification({
            description: 'New Voting Process created successfully!',
            type: 'success',
          });
          getVotingProcesses(address);
        }
  ...
  return (
    <div className="max-w-5xl my-0 mx-auto text-center text-center">
      {!createVotingProcessesVisible && (
        <Button className="mt-5" onClick={() => setCreateVotingProcessesVisible(true)} label="Create Voting Process" />
      )}
      ...
      {votingProcesses.length > 0 && (
        <>
          ...
          {votingProcesses.map((p, i) => (
            <VotingProcess process={p} key={`Process-${i}`} voting={true} />
          ))}
        </>
      )}
    </div>
