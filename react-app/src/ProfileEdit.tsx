...
const ProfileEdit = (props) => {
  ...
  useEffect(() => {
    ...
    if (currentProfile) {
      ...
      if (currentProfile.image) setPreviewSrc(currentProfile.image);
      if (currentProfile.cover) setCoverSrc(currentProfile.cover);
    } else {
      setUsername('');
      setPreviewSrc('');
      setCoverSrc('');
  ...
  if (!web3Context.library) return <div>Connect your wallet</div>;
  ...
  return (
    ...
    <form onSubmit={updateProfile} className="grid grid-cols-1 place-items-center">
      ...
      <input
        ...
        style={{display: 'none'}}
        accept="image/x-png,image/gif,image/jpeg"
        onChange={(event) => handleCoverSelection(event)}
        name="file"
        type="file"
        ref={coverInput}
      />
      ...
      <div>
        <Button type="submit" label={currentProfile ? 'Update Profile' : 'Sign Up'} />
      </div>
    </form>
    /* </CeramicProvider>) */
  );
