...
export const Governance: FC = () => {
  ...
  async function handleSubmit(e) {
    ...
    const stakingTokenContract: Contract = new ethers.Contract(
      CREATE_TOKEN_ADDRESS,
      creaton_contracts.erc20.abi,
      signer
    );
  ...
  async function stakeAndMint(
    ...
    stakingAmount: ethers.BigNumber,
    stakingTokenAddr: string,
    recipientAddr: string
  ) {
    const reactionContract: Contract = new ethers.Contract(
      reactionContractAddr,
      creaton_contracts.ReactionToken.abi,
      signer
    );
  ...
  return (
    <form onSubmit={handleSubmit} className="grid grid-cols-1 place-items-center w-max m-auto py-10 text-white">
      ...
      <p className="mb-10 w-2/3">
        You will stake {createAmount} $CREATE to generate {sgtAmount} ${sgtSymbol}. Your $CREATE tokens will be fully
        streamed back to you in 1 season (4 months)
      </p>
