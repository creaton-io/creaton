...
export const Notification = ({type = 'success', label, icon, description, close}: any) => {
  ...
  return (
    <div
      className={clsx('flex shadow-lg rounded-lg p-6 items-center', defaultSetting.background)}
      style={{width: 'fit-content'}}
    >
      <div
        className={clsx('flex justify-center items-center rounded-full w-12 h-12 text-lg', defaultSetting.iconClass)}
      >
        <Icon name={icon || defaultSetting.icon} />
      </div>
