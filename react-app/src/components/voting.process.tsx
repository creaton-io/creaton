...
export const VotingProcess: FC<VotingProcessProps> = ({process, voting}) => {
  ...
  async function handleVote(e) {
    ...
    try {
      ...
      const creatorVotingContract: Contract = new ethers.Contract(
        process.contract,
        creaton_contracts.creator_voting_process.abi,
        signer
      );
        ...
        <div className="p-8">
          <div className="flex-1 flex flex-col justify-between">
            <div className="flex items-center justify-between">
              <h4 className="text-lg font-semibold text-white">{process.question}</h4>
              <div className="flex justify-between">
                <div className="mr-5">{/* Like, votes or something here? */}</div>
              </div>
            </div>

            <p className="text-left text-white">{process.description}</p>
            ...
            <div className="text-left text-white mt-3">
              ...
              {process.answers.map((a) => (
                <div key={a.id}>
                  <h5 className="ml-2">
                    {a.answer}: {a.countVotes.toString()}
                  </h5>
                </div>
              ))}
            </div>
            ...
            {voting && (
              <div
                className={`${votingModalVisible ? '' : 'hidden'} absolute z-50 p-3 rounded left-2/4 text-left`}
                style={{
                ...
                <form onSubmit={handleVote} className="grid grid-cols-1 place-items-left w-max m-auto text-white">
                  ...
                  {process.answers.map((a) => (
                    <div key={a.id}>
                      ...
                      <label htmlFor={a.id} className="ml-1">
                        {a.answer}
                      </label>
                    </div>
                  ))}
            ...
            {voting && (
              <Button
                className={`${votingModalVisible ? 'hidden' : ''} mt-5`}
                type="submit"
                label="Vote"
                onClick={() => setVotingModalVisible(!votingModalVisible)}
              />
            )}
          </div>
        </div>
