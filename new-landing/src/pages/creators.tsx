	...
	return (
		<MainLayout theme={props.theme} handleSwitch={props.handleSwitch}>
			<div className='container flex flex-col mx-auto'>
				<h1 className={`${props.theme === 'dark' ? 'text-white' : 'text-indigo-900'} text-xl font-bold pt-4 pb-2`}>Creators</h1>
				<div className='flex justify-between'>
					<p className={`${props.theme === 'dark' ? 'text-white opacity-80' : 'text-gray-500'} text-sm`}>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod</p>
					<div className='relative'>
						<input type='text' className={`${props.theme === 'dark' ? 'bg-transparent' : ''} w-80 rounded-lg text-indigo-900 ${props.theme === 'dark' ? 'border-border-200' : 'border-border-100'} border-solid border px-3 py-2`} placeholder='Search for creators...' />
						 <FaSearch className='absolute top-3 right-2 w-5 h-5' fill={`${props.theme === 'dark' ? '#FFFFFF' : '#312880'}`} />
					</div>
				</div>
