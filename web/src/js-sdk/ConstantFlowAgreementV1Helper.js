...
module.exports = class ConstantFlowAgreementV1Helper {
   ...
   */
  async createFlow({superToken, sender, receiver, flowRate, userData, onTransaction = () => null}) {
    ...
    const tx = await this._sf.host
      .callAgreement(
        this._cfa.address,
        this._cfa.createFlow(superTokenNorm, receiverNorm, flowRateNorm, '0x'),
        userData,
        {
          from: senderNorm,
        }
   ...
   * @return {Promise<Transaction>} web3 transaction object
   */
  async updateFlow({superToken, sender, receiver, flowRate, userData, onTransaction = () => null}) {
    const superTokenNorm = await this._sf.utils.normalizeTokenParam(superToken);
    const senderNorm = await this._sf.utils.normalizeAddressParam(sender);
    const receiverNorm = await this._sf.utils.normalizeAddressParam(receiver);
    const flowRateNorm = this._sf.utils.normalizeFlowRateParam(flowRate);
    ...
    const tx = await this._sf.host
      .callAgreement(
        this._cfa.address,
        this._cfa.contract.updateFlow(superTokenNorm, receiverNorm, flowRateNorm, '0x'),
        userData,
        {
          from: senderNorm,
        }
   ...
   * @return {Promise<Transaction>} web3 transaction object
   */
  async deleteFlow({superToken, sender, receiver, by, userData, onTransaction = () => null}) {
    const superTokenNorm = await this._sf.utils.normalizeTokenParam(superToken);
    const senderNorm = await this._sf.utils.normalizeAddressParam(sender);
    const receiverNorm = await this._sf.utils.normalizeAddressParam(receiver);
    ...
    const tx = await this._sf.host
      .callAgreement(
        this._cfa.address,
        this._cfa.contract.deleteFlow(superTokenNorm, senderNorm, receiverNorm, '0x'),
        userData,
        {
          from: byNorm,
        }
