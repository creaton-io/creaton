...
const contractNames = require('../contracts-mumbai.json');
const abis = require('../abi');

const mockContractNames = ['SuperfluidMock', 'SuperTokenMockFactory', 'SuperTokenFactoryMock'];

const loadContracts = ({isTruffle, useMocks, web3Provider, from}) => {
  const allContractNames = [...contractNames, ...(useMocks ? mockContractNames : [])];
  try {
    let contracts = {};
    if (!isTruffle) {
      try {
        console.debug('Using SDK in an external or non-truffle environment');
